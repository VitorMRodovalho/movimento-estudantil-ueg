 ...
 */
/**
 * @version $Revision: 1.56 $ $Date: 2004/12/08 13:56:02 $
 * @package GalleryCore
...
class platform {
    ...
    /**
     * Opens a file with locking
     *
     * @param string a filename
     * @param boolean do we want to lock in exclusive (write) mode?
     * @param string the mode for fopen()
     * @return array object GalleryStatus
     *               resource the opened filehandle (if everything went ok)
     * @access public
     */
    function openAndLock($filename, $lockExclusive = false, $fopenMode = 'rb') {
	if (true) {
	    $fd = $this->fopen($filename, $fopenMode);
	    if (! $fd) {
			sprintf('Can\'t open file (%s) in mode (%s)', $filename, $fopenMode);
	    }
	    $lockMode = ($lockExclusive ? LOCK_EX : LOCK_SH) | LOCK_NB;
	    $i = 0;
	    $locked = false;
	    $wouldBlock = false;
	    while (! $locked && $i++ < 20) {
		/* Try to get a lock 20 times */
		$this->flock($fd, $lockMode, $wouldBlock);
		if (! $wouldBlock) {
		    $locked = true;
		} else {
		    usleep(rand(2,50));
	    ...
	    if ($wouldBlock) {
		/* Still no lock? Somethings wrong... */
			sprintf('Failed to get a%s to (%s)', ($lockExclusive ? 'n exclusive' : ' shared'), $filename);
	    }
	} else {
	    /* TODO: If we can't flock... */
	    $fd = $this->fopen($filename, $mode);	    
	}
	return array(GalleryStatus::success(), $fd);
    }

    /**
     * Close a file opened with openAndLock()
     *
     * @param resource a filehandle
     * @return object GalleryStatus
     * @access public
     */
    function closeAndUnlock($fd) {
	if (true) {
	    $this->flock($fd, LOCK_UN, $wouldBlock);
	} else {
	    /* TODO: Here we should do stuff if flock() is not supported */
	}
	$this->fclose($fd);
    ...
    /**
     * Write some data to a file using exclusive locks
     *
     * @param filename
     ...
     * @access public
     */
    function concurrentWrite($filename, $data) {
	$size = strlen($data);
	if (! $this->file_exists($filename)) {
	    /* We are the first to create the file, just do it! */
	    list ($ret, $fd) = $this->openAndLock($filename, true, 'wb');
	    if ($ret->isError()) {
		return $ret->wrap(__FILE__, __LINE__);
	    }
	    $bytesWritten = $this->fwrite($fd, $data, $size);
	} else {
	    /* File already exists, open read-write to be able to fseek to 0 */
	    list ($ret, $fd) = $this->openAndLock($filename, true, 'r+b');
	    if ($ret->isError()) {
		return $ret->wrap(__FILE__, __LINE__);
	    }
	    /* Move back to beginnnig of the file and write our stuff */
	    if ($this->fseek($fd, 0) != 0) {
		    sprintf('Could not seek to beginning of (%s)', $filename);
	    }
	    $bytesWritten = $this->fwrite($fd, $data, $size);
	    if (! $this->ftruncate($fd, $size)) {
		    sprintf('Could not truncate file (%s)', $filename);
	    }
	}
	$ret = $this->closeAndUnlock($fd);
	if ($ret->isError()) {
	    return $ret->wrap(__FILE__, __LINE__);
	}
	if ($bytesWritten != $size) {
	    sprintf('Failed to write complete file (%s)', $filename);
	}
	return true;
    }

    /**
     * Read some data to a file using shared locks
     *
     * @param filename
     * @return array object GalleryStatus
     ...
     * @access public
     */
    function concurrentRead($filename) {
	$this->clearstatcache();
	$size = $this->filesize($filename);
	list ($ret, $fd) = $this->openAndLock($filename, 'rb');
	if ($ret->isError()) {
	    return array($ret->wrap(__FILE__, __LINE__), null);
	}
	$data = $this->fread($fd, $size);
	$ret = $this->closeAndUnlock($fd);
	if ($ret->isError()) {
	    return array($ret->wrap(__FILE__, __LINE__), null);
	}
	 ...
	 */
	$tmp = GalleryCoreApi::convertExtensionToMime($extension);
    ...
    /**
     * Expand all symbolic links and resolve references to '/./', '/../' and extra '/' characters in the input
     * path and return the canonicalized absolute pathname.   The resulting path will have no symbolic
     * link, '/./' or '/../' components. [cribbed from http://php.net/realpath]
     *
     ...
     *
     * Attempts to set the access and modification time of the file named by filename to the value given by
     * time. If the option time is not given, uses the present time. This is equivalent to what utime
     * (sometimes referred to as utimes) does. If the third option atime is present, the access time of the
     * given filename is set to the value of atime. Note that the access time is always modified, regardless
    ...
    /**
     * Retrun a boolean specifying whether or not this platform can perform

...
if($type == 'quicksearch'){
	...
	$database->setQuery("SELECT DISTINCT img.imgid AS id, img.catid AS gallery_id FROM #__zoomfiles AS img "
    ...
    while($row2 = mysql_fetch_object($zoom->_result)){
        ...
        if($zoom->_toolbox->searchPdf($pdf_doc, $suchstring)){
            $images[] = $pdf_doc;
	...
	for($counter = $startRow; $counter <= $endRow; $counter++){
		...
		if ($zoom->_gallery->isMember() || $image->isMember()) {
			...
			if (!$zoom->_CONFIG['popUpImages']){
				...
				?>
				<td align="left" width="<?php echo $zoom->_CONFIG['size'];?>">
				<a href="#" onClick="window.open('components/com_zoom/view.php?popup=1&catid=<?php echo $zoom->_gallery->_id;?>&key=<?php echo $imgKey;?>&isAdmin=<?php echo $zoom->_isAdmin;?>&hit=1', 'win1', 'width=<?php if($size[0]<450){ echo "450";}elseif($size[0]>$zoom->_CONFIG['maxsize']){ echo $zoom->_CONFIG['maxsize'] + 50;}else{ echo $size[0] + 40;} ?>, height=<?php if($size[1]<350){ echo "350";}elseif($size[1]>$zoom->_CONFIG['maxsize']){ echo $zoom->_CONFIG['maxsize'] + 50;}else{ echo $size[1] + 100;} ?>,scrollbars=1').focus()">
				<?php
			}
			echo ("<img src=\"".$image->_thumbnail."\" border=\"0\"></td>\n"
			 . "\t\t\t<td width=\"10\"></td>\n"
			 . "\t\t\t<td align=\"left\">\n"
			...
			if($zoom->_CONFIG['ratingOn']){
				if($image->_votenum!=0){
					...
					?>
					<img src="components/com_zoom/images/rating/rating<?php echo $rating;?>.gif" border="0"> (<?php echo $image->_votenum;?>
					<?php
					if($image->_votenum==1)
						echo _ZOOM_VOTE.")<br />\n";
					else
